...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Якщо музика вже грає (з іншої сторінки), не перезапускаємо
    if (!localStorage.getItem('musicStarted')) {
        localStorage.setItem('musicStarted', 'true'); // Щоб не запускалось після оновлення
        fadeInMusic();
    } else {
        audio.play().catch(() => console.log("Не вдалося відтворити музику."));
    }
