window.onload = function() {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    function adjustGameSize() {
      let gameContainer = document.querySelector(".field");
      if (!gameContainer) return;
  
      let screenWidth = window.innerWidth;
      let screenHeight = window.innerHeight;
      
      let size = Math.min(screenWidth * 0.9, screenHeight * 0.7); // Автоматичний розмір гри
  
      gameContainer.style.width = `${size}px`;
      gameContainer.style.height = `${size}px`;
  
      let cells = document.querySelectorAll(".cell, .grid_cell");
      let cellSize = size / 4 - 10; // Ділимо на 4 для сітки 4x4
